...
5. Decide deliberately whether to **replace** `state`'s copy or **merge** with it — don't default to merging. Events scored by a run that never completed a real email send (e.g. a `workflow_dispatch` test) were never actually reported to the client and should be dropped, even though the pipeline itself considers them already-seen.
