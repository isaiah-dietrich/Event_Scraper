...
```
Flags combine freely (`--test` picks *which URLs*, `--fresh` picks whether existing results are cleared first — independent axes). See README.md's combination table for the exact input/output pairing of each combo.
...
No automated test suite is maintained on `main` (it lives on the `tests` branch instead) — verify changes by running the pipeline directly (e.g. `python run.py --test`).
...
Every run writes through `append_rows`: the permanent, deduped Events/Rejected Events workbook.
